        ...
        >
            <div class="snackbar-wrapper md-layout md-alignment-center-space-between md-size-100">
                ...
                <div class="snackbar-action-wrapper  ml-auto md-alignment-center-right ">
                    <md-button v-if="selectedItems.length === getPatientAnamnesis.length" class="md-simple" @click="unselectAll()">
                        {{ $t(`${$options.name}.unselect`) }}
                    </md-button>
                    <md-button v-else class="md-simple" @click="selectedItems = getPatientAnamnesis">
...
export default {
    ...
    methods: {
        sortBytypes(currentSort, currentSortOrder) {
            const vm = this;
            const val = vm.getPatientAnamnesis.sort((a, b) => {
