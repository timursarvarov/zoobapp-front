...
<template>
  <div class="md-layout md-gutter">
      ...
      >
        <md-table-toolbar>
         ...
         <md-table-empty-state
          md-label="No treatment found"
          :md-description="`No treatment found. Scroll top, and create new treatment.`">
        ...
        <md-table-row slot="md-table-row" slot-scope="{ item }">
          ...
          <md-table-cell md-label="Treatment" md-sort-by="title">
            {{ item.title }}<br/>
          ...
          <md-table-cell md-sort-by="author" md-label="Treatmentd by">
          ...
          <md-table-cell class="actions" md-label="Actions">
            ...
            <md-button
              v-if="ifTreatmentHasLocations(item.teeth)"
    ...
    <div class="md-layout-item md-layout md-alignment-right-center md-size-100 ml-auto">
      <md-button class="md-warning ml-auto">Delete</md-button>
      <md-button class="md-success">Set Primary</md-button>
    </div>
  </div>
</template>
<script>
    ...
    export default {
      ...
      data() {
        return {
          ...
          searchQuery: '',
          propsToSearch: ['code', 'treatment', 'date', 'author', 'title'],
          searchedData: [],
          fuseSearch: null,
        };
      },
      computed: {
        ...
        setecltedTeeth() {
          if (!this.isEmpty(this.selectedTreatment)) {
            return Object.keys(this.selectedTreatment.teeth);
          }
          return [];
        },
        tableData() {
      ...
      methods: {
        ...
        saveTreatment(d) {
          const treatmentEdited = this.selectedTreatment;
          treatmentEdited.teeth = d.teeth;
          treatmentEdited.description = d.description;
          this.$store.dispatch(PATIENT_TREATMETS_UPDATE, {
            treatment: treatmentEdited,
          });
          this.recalculateJaw();
        },
        ifTreatmentHasLocations(teeth) {
          let show = false;
        ...
        deleteRow(item) {
          ...
          const indexToDeleteOrigin = this.patientTreatments.findIndex(
            treatment => treatment.id === item.id,
          );
          if (indexToDeleteOrigin >= 0) {
            this.patientTreatments.splice(indexToDeleteOrigin, 1);
          }
        },
      },
      mounted() {
        ...
        this.fuseSearch = new Fuse(this.tableData, {
          keys: ['treatment', 'code', 'author', 'date', 'title'],
          threshold: 0.3,
        });
      },
      watch: {
