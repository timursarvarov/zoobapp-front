  ...
  methods: {
    ...
    chooseLocation(toothId, location) {
      ...
      if (this.selectedDiagnose.locations.hasOwnProperty(location)) {
        ...
        // записываем последнюю редактируемую локации для быстрого выбора локации
        ...
        const addLocation = this.selectedDiagnose.addLocation
  ...
  created: function() {
    ...
    if (this.selectedTeeth.length > 0) {
      if (
        ...
        // если в выбранном диагнозе выбрана лишь одна локация то выбери ее
      }
      if (Object.keys(this.selectedDiagnose.locations).length === 1){
        const singleLocation = Object.keys(this.selectedDiagnose.locations)[0]
         this.selectedTeeth.forEach(element => {
          this.chooseLocation(element, singleLocation)
        });
      }
    console.log(this.selectedDiagnose.locations)
    this.triger = !this.triger;
    }
    }
    // this.getDiagnose();
