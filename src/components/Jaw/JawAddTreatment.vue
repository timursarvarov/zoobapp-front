      ...
      <div class="md-toolbar-row">
        ...
        <div class="md-toolbar-section-end">
          ...
          >
            ...
            <md-tooltip
              v-if="'locations' in originalTreatment"
              md-delay="1000"
            >
              Toggle teeth
            </md-tooltip>
...
<script>
    ...
    export default {
      ...
      methods: {
        ...
        toothClick(toothId) {
          if (this.isEmpty(this.originalTreatment.locations)) {
            this.toggleTooth(toothId);
          }
        },
        toggleTooth(toothId) {
          if (toothId in this.treatment.teeth) {
            delete this.treatment.teeth[`${toothId}`];
          } else {
            this.treatment.teeth[`${toothId}`] = {};
          }
          this.calculateJaw();
        },
        isSelected(toothId) {
          if (Object.keys(this.treatment.teeth).includes(`${toothId}`)) {
            return 'selected';
          }
          return false;
        },
        getToothClasses(toothId, location) {
          const toothClasses = {
            ...
            selected:
              this.getNestedProperty(this.treatment, 'teeth', toothId, location)
              !== undefined,
        ...
        shouldHideLocation(toothId, location) {
          ...
          if (!this.preferj) {
            return !this.getNestedProperty(this.originalTreatment, 'view', location);
          }
        ...
        setLastLocationForAllTeeth() {
          if (!this.isEmpty(this.originalTreatment.locations)) {
            ...
            if (this.selectableTeeth.length === this.numProps(this.treatment.teeth)) {
        ...
        getTeethWithSameLocation(location) {
          return Object.keys(this.treatment.teeth).filter(
            toothId => location in this.treatment.teeth[toothId],
          );
        },
        ...
        chooseLocation(toothId, location) {
          ...
          if (this.hasProp(this.originalTreatment.locations, location)) {
            // если выбранный зуб не находиться в списке зубов дигноза
            if (!this.hasProp(this.treatment.teeth, toothId)) {
              this.treatment.teeth[toothId] = {};
            }
            ...
            const { addLocation } = this.originalTreatment;
            this.treatment = addLocation(location, this.treatment, this.originalTreatment, toothId);

            // удаляем зуб из диагноза если у него нет локациий
            if (this.isEmpty(this.treatment.teeth[toothId])) {
              delete this.treatment.teeth[toothId];
            }
          }
          this.calculateJaw();
        ...
        getTreatment() {
          this.$emit('updateDiagonoseParams', this.treatment);
        },
        setLocationOnLoad(location, toothId, value) {
          if (value !== undefined) {
            ...
            this.treatment.teeth[toothId][location] = value;
          }
        },
        ...
        setAllTeethOnLoad() {
          const { setOnLoad } = this.originalTreatment;
          // если нет локаций
        ...
        setAllEditableTeethOnLoad() {
          Object.keys(this.selectedTreatment.teeth).forEach((toothId) => {
            const tooth = this.selectedTreatment.teeth[toothId];
            // если нет локаций
        ...
        setInitiallyLocations(selectableTeethExtraChoose) {
          ...
          if (this.selectableTeeth.length > 0) {
            ...
            if (!this.isEmpty(this.originalTreatment.locations)) {
              ...
              if (!this.isEmpty(this.selectedTreatment.teeth)) {
          ...
          this.selectableTeeth.forEach((toothId) => {
            ...
            Object.keys(this.defaultLocations).forEach((location) => {
              ...
              jaw[toothId][location] = {
                hide: this.isHidingLocation(toothId, location),
      ...
      created() {
        ...
        this.handleResize();
        this.treatment.code = this.selectedTreatment.code;
        this.treatment.title = this.selectedTreatment.title;
