<template>
  <div class="wizard-container">
    <form @submit.prevent>
      ...
      with one of the next bright colors: "green",
      "orange", "red", "blue"       -->
      <md-card
          ...
          >
            ...
            >
              <a
                ...
                :class="[{'disabled-wizard-link': !tab.checked},
                {active: tab.active}, {checked: tab.checked}]"
                data-toggle="tab"
              >
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      movingTabStyles() {
        const res1 = this.tabLinkWidth * this.activeTabIndex - 8;
        const res2 = this.tabLinkWidth * this.activeTabIndex + 8;
        const res3 = this.tabLinkWidth * this.activeTabIndex;
        const res4 = this.activeTabIndex === this.tabCount - 1 ? res2 : res3;

        let translateXValue = this.activeTabIndex === 0 ? res1 : res4;

        // let translateXValue =        this.activeTabIndex == 0
    ...
    methods: {
      addTab(tab) {
        const theTab = tab;
        const index = this.$slots.default.indexOf(theTab.$vnode);
        const tabTitle = theTab.title || '';
        theTab.tabId = `${tabTitle.replace(/ /g, '')}${index}`;
        if (!this.activeTab && index === 0) {
          theTab.active = true;
          theTab.checked = true;
        }
        if (this.activeTab === theTab.name) {
          theTab.active = true;
          theTab.checked = true;
        }
        this.tabs.splice(index, 0, theTab);
      },
      removeTab(theTab) {
        const { tabs } = this.tabs;
        const index = tabs.indexOf(theTab);
        if (index > -1) {
          tabs.splice(index, 1);
        }
      },
      validate(theTab) {
        const tabToValidate = theTab || this.activeTab;
